...
Feature: API Gateway Security
  ...
  Scenario: CORS preflight returns allowed headers
    When I send an OPTIONS request to "/api/v1/auth/login" from origin "https://admin.ecom.app"
  ...
  Scenario: CORS from unknown origin is blocked
    When I send an OPTIONS request to "/api/v1/auth/login" from origin "https://evil-site.com"
