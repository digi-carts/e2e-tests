...
Feature: Catalog API
  ...
  Scenario: Upload image without auth returns 401
    When I POST a multipart file to "/api/v1/catalog/products/abc123/images"
    Then the response status should be 401

  Scenario: Upload image with wrong MIME type returns 400
    Given I have a valid JWT token for role "ADMIN"
    When I POST a multipart file with content type "text/plain" to "/api/v1/catalog/products/abc123/images"
